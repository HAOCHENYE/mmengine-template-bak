...
from detrex.layers import (FFN, MLP, BaseTransformerLayer, MultiheadAttention,
                           MultiScaleDeformableAttention,
                           TransformerLayerSequence, get_sine_pos_embed)
from detrex.utils import inverse_sigmoid

from mmengine_template.registry import MODELS


class _MultiScaleDeformableAttention(MultiScaleDeformableAttention):

    def init_weights(self):
        if getattr(self, '_is_init', False):
            return
        result = super().init_weights()
        setattr(self, '_is_init', True)
        return result
    ...
    def __init__(
        ...
        super().__init__(
            transformer_layers=BaseTransformerLayer(
                attn=_MultiScaleDeformableAttention(
                    embed_dim=embed_dim,
                    num_heads=num_heads,
    ...
    def forward(
        ...
        **kwargs,
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            query_sine_embed = get_sine_pos_embed(reference_points_input[:, :,
                                                                         0, :])
...
class DINOTransformer(nn.Module):
    ...
    def __init__(
        ...
        if self.learnt_init_query:
            self.tgt_embed = nn.Embedding(self.two_stage_num_proposals,
                                          self.embed_dim)
    ...
    def _init_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, _MultiScaleDeformableAttention):
                m.init_weights()
    ...
    def forward(
        ...
        **kwargs,
        ...
        for lvl, (feat, mask, pos_embed) in enumerate(
                zip(multi_level_feats, multi_level_masks,
                    multi_level_pos_embeds)):
            bs, c, h, w = feat.shape
            spatial_shape = (h, w)
        ...
        enc_outputs_coord_unact = (
            ...
            output_proposals)  # unsigmoided.

        topk = self.two_stage_num_proposals
        topk_proposals = torch.topk(
            enc_outputs_class.max(-1)[0], topk, dim=1)[1]

...
class DINO(BaseModel):
    """Implement DAB-Deformable-DETR in `DAB-DETR: Dynamic Anchor Boxes are
    Better Queries for DETR.
    ...
    def __init__(self,
        ...
        if isinstance(transformer, dict):
            transformer = MODELS.build(transformer)
        if isinstance(position_embedding, dict):
            position_embedding = MODELS.build(position_embedding)
        if isinstance(criterion, dict):
            criterion = MODELS.build(criterion)
        ...
        # hack implementation for two-stageinit_weights
    ...
    def prepare_for_cdn(
        ...
        """A major difference of DINO from DN-DETR is that the author process
        pattern embedding pattern embedding in its detector forward function
        and use learnable tgt embedding, so we change this function a little
        bit.
        ...
        positive_idx = (
            torch.tensor(range(len(boxes))).long().cuda().unsqueeze(0).repeat(
                dn_number, 1))
        ...
        if box_noise_scale > 0:
            ...
            rand_sign = (
                torch.randint_like(
                    known_bboxs, low=0, high=2, dtype=torch.float32) * 2.0 -
                1.0)
        ...
        for i in range(dn_number):
            if i == 0:
                attn_mask[single_padding * 2 * i:single_padding * 2 * (i + 1),
                          single_padding * 2 * (i + 1):pad_size, ] = True
            if i == dn_number - 1:
                attn_mask[single_padding * 2 * i:single_padding * 2 *
                          (i + 1), :single_padding * i * 2] = True
            else:
                attn_mask[single_padding * 2 * i:single_padding * 2 * (i + 1),
                          single_padding * 2 * (i + 1):pad_size, ] = True
                attn_mask[single_padding * 2 * i:single_padding * 2 *
                          (i + 1), :single_padding * 2 * i] = True
